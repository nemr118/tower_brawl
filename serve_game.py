#!/usr/bin/env python3
"""
Dual HTTP (8000) & HTTPS (8443) Web Server + WebSocket Real-Time Game Relay (8081).
Connects multiple browser clients (Phones, Laptops, PCs) into the same live match.
"""
...
def ws_client_thread(client_sock, addr):
    ...
    print(f"🎮 Player {assigned_id} connected from {addr} (Total: {len(connected_clients)})")
    ...
    print(f"🔌 Player {assigned_id} disconnected (Remaining: {len(connected_clients)})")
    client_sock.close()
...
def start_ws_server():
    ...
    server.listen(10)
    print(f"⚡ Real-Time WebSocket Relay running on ws://0.0.0.0:{WS_PORT}")
    while True:
...
if __name__ == "__main__":
    ...
    print("\n" + "═"*65)
    print("🎮 TOWERBRAWL MULTIPLAYER SERVER & RELAY RUNNING!")
