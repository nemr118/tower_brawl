...
## Now (2026-09-05)
- **Tagged `v0.0.34`** on `master`, working tree clean at handoff. **Harness 23/23 with `--restart-each`, no Minor**, report `harness_report_v0.0.34.json`.
- **Next: optimisation Step C, cut 1 needs a phone reading.** The cut list from [[optimisation_step_b_profiles]] (PC: about 1 ms of a 16.7 ms frame, server 1.7 % of a core, `sync_pos` 84 % of inbound bytes, `history_status` a third of a player's outbound): (1) the phone main thread, now measurable (`proc=` / `phys_cpu=` on NetStats, v0.0.34); read the S25 Ultra over USB (section 1 of the master sheet) before cutting a script; (2) `history_status` every 2.6 s, send on change or every 10 s; (3) packet count over packet size (coalesce `sync_pos`). Not worth a build: `status_loop`, the 40 log lines, JSON events to binary. One build per cut, gated by the harness. Then `v0.1.0`. Backlog 20 moves up if a playtest gives a `🪤 [JumpTrap]` line.
- **Waiting on a human: one sheet, [[Playtest — Master Validation Suite]]** (v0.0.34). Section 1 the phone frame numbers over USB (S25 Ultra, Pixel, iPad), section 2 the ranger quiver (backlog 1, 6), section 3 the PC reload keyboard probe (backlog 13), section 4 the free-fall feel (backlog 23). It replaced the three old sheets; it stays in `docs/` until every verdict is ticked.
...
- **v0.0.34 "Mobile Frame Telemetry" (optimisation Step C, build 1):** `global.gd` `_ft_sample` sums `Performance.TIME_PROCESS` and `TIME_PHYSICS_PROCESS` every drawn frame; the NetStats `fps` part ends with `proc=X.Xms phys_cpu=Y.Yms` (Godot refreshes both once a second with the slowest frame of that second; printed as the 5 s mean of those), `net_stats_updated` carries `proc_ms` / `phys_cpu_ms`. USB phone console in [[Commands]]. Game unchanged. See [[v0.0.34 - Mobile Frame Telemetry]].
