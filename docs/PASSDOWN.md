...
## Now (2026-09-07)
- **Tagged `v0.0.40`** on `master`, tree clean. **Harness 24/24 with `--restart-each`, no Minor**, report `harness_report_v0.0.40.json`. **The family plays on the laptop** ([[laptop-server]]): `tools/deploy_laptop.sh <ip>` pushes a build, `tools/pull_laptop_logs.sh <ip>` pulls the night's data.
- **Next: tag `v0.1.0` = v0.0.40 with a new number, no code change (decided 2026-09-07).** Runbook: [[release-v0.1.0]]. Gate: sheet section 6 on v0.0.40, then `release/v0.1.0`, `./bump_build.sh minor`, suite 24/24, tag, ff-merge, deploy to the laptop. **v0.1.1** = cut 1 (the phone main thread) or the Android export + PCK loader; decide at the tag (runbook §5). Bundle feels late? `RELAY_BUNDLE_MS = 0`.
- **Waiting on a human: one sheet, [[Playtest — Master Validation Suite]]** (v0.0.40). No console: clock times only (match log 0.1); `tools/stats_table.py --match N` prints the rows. Sections: 1 phone frames, 2 ranger quiver (backlog 1, 6), 3 PC reload keys (backlog 13), 4 shift feel with the anchored floor (backlog 9, 24), 5 bundle feel, 6 v0.1.0 sign-off. Stays in `docs/` until every verdict is ticked.
...
- **v0.0.39 "Anchored Ground" (user decision):** the ground slabs no longer turn; `arena.gd` `_anchor_ground()` reparents them into a `Ground` node at load, the rest turns as a group; the spot picker and `bot_brain.gd` read both nodes. Backlog 22 closed, 24 opened. See [[v0.0.39 - Anchored Ground]].

## Open backlog
`N. **Title** — state — next action — where the detail lives`. Numbers are never reused; closed items are in [[closed]].
3. **Second tab spectating stalls the game tab** — open, mitigated — spectate from a second device — [[PASSDOWN-2026-09-05]].
