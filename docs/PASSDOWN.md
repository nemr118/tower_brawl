...
## Now (2026-09-07)
- **Tagged `v0.0.38`** on `master`, tree clean. **Harness 24/24 with `--restart-each`, no Minor**, report `harness_report_v0.0.38.json`.
- **Next: tag `v0.1.0` = v0.0.38 with a new number, no code change (decided 2026-09-07).** Runbook: [[release-v0.1.0]]. Gate: master sheet section 6 on v0.0.38 (counts because nothing else changes), then `release/v0.1.0`, `./bump_build.sh minor`, suite 24/24, tag, ff-merge. **v0.1.1** = cut 1 (the phone main thread, from the S25 Ultra rows) or the Android export + PCK loader; decide at the tag (runbook §5). Bundle feels late? `RELAY_BUNDLE_MS = 0` in `serve_game.py`.
- **Waiting on a human: one sheet, [[Playtest — Master Validation Suite]]** (v0.0.38). No console: the sheet asks for clock times (match log 0.1); `tools/stats_table.py --match N` prints the rows. Sections: 1 phone frames, 2 ranger quiver (backlog 1, 6), 3 PC reload keys (backlog 13), 4 shift ride feel (backlog 9), 5 bundle feel, 6 v0.1.0 sign-off. Stays in `docs/` until every verdict is ticked.
- **Ideas, not scheduled:** replay skip key, virtual stick, touch buttons, spread air spawns, spawn shield, death animations, zoomed replay, sound; later a headless Godot sim server replaces the relay, native apps with a PCK loader, Dead Cells style 3D fighters. Detail: [[PASSDOWN-2026-09-05]].
- **v0.0.38 "Solid Shift Restored" (user decision):** the platforms are solid during the arena shift again; `arena.gd` `const SHIFT_SOFT_PLATFORMS = false` gates the v0.0.32 softening (a switch for arena configurations). The free-fall tumble felt terrible; backlog 9 is open again. See [[v0.0.38 - Solid Shift Restored]].
- **v0.0.37 "Client Stats Reporting" (playtest tooling):** every client sends its NetStats numbers to the server every 5 s (`client_stats`, 543 B, never relayed); the server keeps the newest per socket (`status.json`, deck column Screen) and appends every card plus match / round markers to `client_stats.jsonl`; `tools/stats_table.py` prints a row per device per match. See [[v0.0.37 - Client Stats Reporting]].
...
9. **A fighter rides the arena shift out of the top of the screen** — reopened in v0.0.38 (user decision) — solve with the arena configurations; candidate: option B, wrap a fighter carried above the top edge to the bottom — [[v0.0.31 - Top Edge Probe]], [[v0.0.38 - Solid Shift Restored]].
...
18. **Archer kit revisit (the proper fix behind backlog 1, 6, 15)** — design call — slow regen, steal arrows, arrows in the join snapshot, ammo on other screens, the two-arrow frame — [[v0.0.30 - Ranger Rejoin Quiver]].
19. **Fill-with-bots button** — design call — the server would start headless bot processes.
...
- `tools/watch_server.py` + `tools/deck_input.py`: the deck (`tbdash`), reads `status.json` and `harness_state.json` only. `tools/stats_table.py`: rows per device from `client_stats.jsonl`. `tools/chaos_bots.py`: protocol oracle, 24 scenarios, fuzz, soak, `GodotClient` fleet runner, the gates.
