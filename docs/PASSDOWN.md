...
- **v0.1.2 "Cut 1" built** (build 2), harness 25/25 with `--restart-each`, no Minor, report `harness_report_v0.1.2.json`, PC service restarted, deployed on the laptop ([[laptop-server]]). Last tag: `v0.1.0` (v0.1.1 and v0.1.2 untagged).
- **Waiting on a human:** [[Playtest — Master Validation Suite v0.1]], two phones on the laptop: sections 1 to 4 (the controller, the kill cam circle, the speed burst, the respawn mash) and section 5 (cut 1: the first replay and the lobby; the numbers are the `"kind":"replay"` records and the round 1 cards).
- **Next:** the arena configurations (backlog 9, 24). The Android app + PCK loader is an idea, not a plan.
- **Ideas, not scheduled:** a twin-stick toggle, `stats_table.py --devices` printing the screen size (Chrome hides the model) and a `--replays` view, the Speed line as a server card, replay skip key, sound; later a headless Godot sim server. Detail: [[PASSDOWN-2026-09-05]].
- **v0.1.2 "Cut 1":** the lobby icons 256 px and loaded once (`ICON_TEX`; were 1024 px, loaded on every roster change), a font warm-up at scene load (`font_warmup.gd`, `🔤 [FontWarm]`: the first replay's 89 ms was its first glyph atlases), the ghost pool, a replay start probe (`start_ms tick_ms first_ms` in the line, the card and `client_stats.jsonl`), the self-respawn net (`🩹 [SelfRespawn]`, `SELF_RESPAWN_S` 1.0), a web client on the PC (`tools/webbot.sh`); `.pck` 21.9 -> 12.9 MB. See [[v0.1.2 - Cut 1]].
- **v0.1.1 "Phone Controller":** round buttons on an arc, nearest-centre hit test; a stick aim ring (`Global.touch_aim`); the replay circles the closing kill (`report_stamp()`); a `🏃 [Speed]` probe. Same day: backlog 27 (`DEATH_DEDUPE_S` 0.6, scenario `respawn_death`) after the night soak's 13 h deadlocked round. See [[v0.1.1 - Phone Controller]].
...
- **Verify:** `godot --headless --path . --script tools/check_scripts.gd -- --no-net`, then `./venv/bin/python tools/chaos_bots.py --restart-each --json docs/harness_report_<ver>.json` (bump first; detached; wait for `25/25`; it restarts the PC service per scenario: leave the service alone meanwhile).
