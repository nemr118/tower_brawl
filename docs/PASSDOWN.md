...
- **Night soak 2026-09-08 (4 bots on the laptop):** 27 matches, 190 rounds, no "no closing kill" line, server 33 MB after 15 h; then **backlog 27 deadlocked a round for 13 h**. Numbers on the v0.1.1 page.
- **Next: backlog 27 first** (one server constant + a harness scenario + a laptop deploy), then **build 2 = cut 1 proper** from the phones' rows: the first replay's start hitch (89 ms S25 Ultra, 52 ms Pixel) and the lobby's 21 to 35 ms worst frames on the S25; in play the phones sit at 7 to 8 ms, 60 ticks, no hitch. Then the arena configurations (backlog 9, 24). The Android app + PCK loader is an idea, not a plan.
...
27. **A second death 1.2 to 1.5 s after the first is dropped as a duplicate; the fighter stays dead on its screen, alive on the server** — open, netcode, seen once in 1.6 h of 4-bot play (deadlocked a round for 13 h) — `DEATH_DEDUPE_S` 1.5 must be under the 1.2 s respawn delay; a harness scenario; a client self-respawn if its death is not echoed — [[v0.1.1 - Phone Controller]].
28. **A round has no clock: two fighters that never meet play for ever** — design call — a round timer; backlog 27 showed it — [[v0.1.1 - Phone Controller]].
...
- `tools/watch_server.py`: the deck (`tbdash`). `tools/stats_table.py`: rows and match summaries from `client_stats.jsonl`. `tools/tbbot.py`: headless bots (logs in `.bots/`). `tools/chaos_bots.py`: 24 scenarios, the gates.
