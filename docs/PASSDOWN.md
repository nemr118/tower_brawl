...
## Now (2026-09-10)
- **v0.1.5 "Bot Routes" built**, harness 25/25 with --restart-each, no Minor, `harness_report_v0.1.5.json`, deployed on the laptop ([[laptop-server]]). Last tag: `v0.1.0` (v0.1.1 to v0.1.5 untagged).
- **Waiting on a human:** [[Playtest — Master Validation Suite v0.1]] §1 to §7 (§6 the tower, §7 twin sticks) and **§8 (the overnight bot soak, `tools/soak_report.py`)**. 4 devices played v0.1.4 on 2026-09-08 22:34 to 22:47, no verdict written.
- **Next:** the §6 to §8 verdicts, then what they ask for (pace, map, rim and angle numbers), or tower build 2 (themes, moving platforms, traps).
- **Ideas:** stats views, the Speed line as a server card, a replay skip key, sound, a sim server. [[PASSDOWN-2026-09-05]].
- **v0.1.5 "Bot Routes":** every soak stalled (a bot whose target stood above or below it froze; rounds of 8 h and 10 h, [[bot-soak-2026-09-08]]). `bot_nav.gd` (new) makes the layout 25 places to stand and the jumps, drops and walk-offs between them; `bot_brain.gd` follows the route, drops with down first (backlog 31), and has two stall guards (hunt after 20 s with no kill, a detour after 4 s stuck). Tools: `nav_probe.gd`, `soak_report.py`. [[v0.1.5 - Bot Routes]].
- **v0.1.4 "Twin Sticks":** a second phone layout in `touch_controls.gd` (`layout` `arc` / `twin`, saved as `towerbrawl_layout`, a SWAP button): the left stick walks, jumps, dashes past its rim, ducks (its rim = the drop-through); the right stick aims and shoots past its rim. The v0.1.3 duck flickered every frame (a shape change costs one floor frame): `DUCK_GRACE_S` 0.1 in `player.gd`. Tools: `webshot.py`, `duck_probe.gd`. [[v0.1.4 - Twin Sticks]].
...
- `scripts/history_ring.gd` the tape (360 frames), `scripts/replay_player.gd` the VHS replay (211 frames around the closing kill; ghosts pooled at load), `scripts/font_warmup.gd` the glyph warm-up, `scripts/touch_controls.gd` the phone controller (two layouts), `scripts/bot_brain.gd` the personas (`wanderer chaser sniper turtle rusher griefer`). `web/shell.html` the page.
