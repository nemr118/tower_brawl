...
## Now (2026-09-07)
- **Tagged `v0.0.39`** on `master`, tree clean. **Harness 24/24 with `--restart-each`, no Minor**, report `harness_report_v0.0.39.json`.
- **Next: tag `v0.1.0` = v0.0.39 with a new number, no code change (decided 2026-09-07).** Runbook: [[release-v0.1.0]]. Gate: master sheet section 6 on v0.0.39 (counts because nothing else changes), then `release/v0.1.0`, `./bump_build.sh minor`, suite 24/24, tag, ff-merge. **v0.1.1** = cut 1 (the phone main thread, from the S25 Ultra rows) or the Android export + PCK loader; decide at the tag (runbook §5). Bundle feels late? `RELAY_BUNDLE_MS = 0` in `serve_game.py`.
- **Waiting on a human: one sheet, [[Playtest — Master Validation Suite]]** (v0.0.39). No console: the sheet asks for clock times (match log 0.1); `tools/stats_table.py --match N` prints the rows. Sections: 1 phone frames, 2 ranger quiver (backlog 1, 6), 3 PC reload keys (backlog 13), 4 shift feel with the anchored floor (backlog 9, 24), 5 bundle feel, 6 v0.1.0 sign-off. Stays in `docs/` until every verdict is ticked.
...
- **v0.0.39 "Anchored Ground" (user decision):** the ground slabs no longer turn; `arena.gd` `_anchor_ground()` reparents them into a `Ground` node at load (`ANCHORED_PLATFORMS`), the rest turns as a group; `_platform_tops` and `bot_brain.gd` read both nodes. Backlog 22 closed, 24 opened. See [[v0.0.39 - Anchored Ground]].
- **v0.0.38 "Solid Shift Restored" (user decision):** platforms solid during the shift again; `arena.gd` `const SHIFT_SOFT_PLATFORMS = false` gates the v0.0.32 softening. The free-fall tumble felt terrible. See [[v0.0.38 - Solid Shift Restored]].
...
24. **Turning ledges sweep the anchored slab corners mid-turn** — open, accepted for the v0.1.0 playtest — arena configurations: soft turning pieces (`SHIFT_SOFT_PLATFORMS`) or outer ledges inward; sheet 4.1 asks if it hurt anyone — [[v0.0.39 - Anchored Ground]].
