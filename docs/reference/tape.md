...
```
The browser console prints the same `📼 [Tape]` line. The card (`history_status`, client to server, never relayed, kept as `seats[].tape` in `status.json` by `player_tapes`): `frames, span_ms, fps, recording, frozen, round, stamps, closing_seq, last {seq, round, killer, victim, weapon, closing, before, after, fighters}, replay`. **When the card goes out (v0.0.35):** at once when the tape freezes and when the replay starts, ends or is skipped; a kill stamp only marks it dirty and `arena.gd` `_record_tape_frame` sends the dirty card at most once per `TAPE_CARD_MIN_S` (10 s) while recording, so the deck's stamp count can lag up to 10 s. Before v0.0.35 every stamp sent a card (about 380 B from every screen per kill). On the deck: the `Tape` column in SEATS, the inspect line of a round-end column, and the `[TAPE]` events on key `0` ([[deck]]).
