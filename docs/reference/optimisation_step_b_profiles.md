...
- **The tape card is the biggest thing a player sends,** bigger than its own movement. `history_status` is 380 B every 2.6 s from every screen, all match long, whether or not the tape changed. That is the one outbound cut that shows in the numbers. `bot_status` only exists on brains.
  - **Correction (v0.0.35, Step C build 2):** there is no 2.6 s timer. `arena.gd` sent the card on every kill stamp (plus the freeze and the replay start / skip / end), and 0.38 a second was this fleet's kill rate. The cut is the per-kill send; see [[v0.0.35 - Tape Card on Change]] for the before and after.
...
2. **`history_status` on every kill from every screen** (not a timer, see the correction above). A third of a player's outbound bytes for a deck column. Done in v0.0.35: a stamp marks the card dirty, sent at most once per 10 s; freeze and replay send at once.
3. **Packet count, not size.** 41 packets a second into each client; header cost is five times the payload. Coalescing two puppets' `sync_pos` into one frame would halve the count. A bigger change than it looks (the relay sends per sender). Done in v0.0.36: the relay holds the samples for 50 ms and sends one `sync_bundle` per client; see [[v0.0.36 - Relay Packet Coalescing]] and [[optimisation_step_c_bundle]].
