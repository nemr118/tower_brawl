...
godot --headless --path . -- --autojoin --name=Bot --ai=sniper --ai-seed=7 --ai-difficulty=0.7 --latency-ms=120   # or: tbbot add sniper
```
...
- `🧗 [TopEdge]` while a local fighter above `y = -20` touches something (backlog 9). `🏃 [Speed]` once a second while the local fighter moves sideways over 300 px/s outside a dash (backlog 25). `🧭 [Spawn]` per round, `🪤 [JumpTrap]` on an ignored floor jump, `🧠 [Bot ...]` every 5 s for a brain, `📼 [Tape]` / `📼 [Replay]` ([[tape]], [[replay]]).
...
```
`server.log` never rotates on the PC: filter by the last "Game version" line. `client_stats.jsonl` rotates at 10 MB. Deck keys: [[deck]].
