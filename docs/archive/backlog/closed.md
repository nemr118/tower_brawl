...
9. **A fighter rides the arena shift out of the top of the screen** — fixed in **v0.0.32** (option A): `arena.gd` `_set_platforms_solid(false)` zeroes `collision_layer` on every `StaticBody2D` under `Platforms` while the stage turns, restored at the timer end and in `_finish_spin_now`; cause found in v0.0.31 (the outer platforms pass 29 to 36 px above the top edge mid-turn); feel check [[Playtest v0.0.32 — Free-Fall Arena Shift]] — [[v0.0.32 - Free-Fall Arena Shift]], [[v0.0.31 - Top Edge Probe]].
