...
| Build shown in the lobby (must be v0.0.41, the release candidate) | |
...
| During the replay (the REW / PLAY / SLOW / STOP strip) | | | |
...
- [x] **Closed 2026-09-07 without a play:** since v0.0.37 every device sends these numbers to the server on its own, so this section has no human step left; cut 1 (the reason for it) moved to the v0.1.1 decision. The S25 Ultra rows will come from any future game (`tools/stats_table.py --devices`).
...
- [x] **Closed 2026-09-07 without a play:** the ranger fixes (v0.0.30) are covered by the `ranger_rejoin` harness scenario; the human check was a look at the ammo count after a real reload. Backlog 1 and 6 stay closed; reopen one only if a real game shows the quiver wrong after a reload.
...
- [x] **Closed 2026-09-07 without a play:** the keyboard lockout was seen once, at the June playtest, and never since in weeks of play. Backlog 13 stays open as a low line (not a release gate); the `keys=` and `focus=` numbers are in every card now, so the next time it happens the clock time is enough.
...
- [x] **The anchored floor feels right; the ledge ride and the slab-corner sweep are nits:** ship v0.1.0 with it. (User decision 2026-09-07 after playing v0.0.39; the free fall of v0.0.32 was reverted in v0.0.38 because it felt terrible.) Backlog 9 and 24 stay open for the arena configurations.
...
### 5.1 Setup
- [ ] Build in the lobby is v0.0.41 or later (an older build is refused with VERSION MISMATCH).
...
- [x] **The puppets feel the same and `IN` is about half of 40 pkt/s:** the bundle stays. Step C cut 3 is closed. (Match 6 on the laptop, 2026-09-07 13:56 to 14:00, PC on Chrome vs a bot, 49 cards: `IN` 16.0 pkt/s, 74 bundles per 5 s, jitter 22.3 px/s mean, 9 snaps in 9 rounds, stall 0.2 %, extrap 0.2 %; nothing felt late in several builds of play.)
...
**Why.** v0.1.0 is the first minor version: v0.0.41 with a new number and no code change ([[release-v0.1.0]]). Sections 1 to 5 each look at one feature. This section looks at the whole game the way a player does: one full match from lobby to the crown, with a spectator watching, on the devices you have. The harness cannot judge this, a person can. There is no sound in this build (sound is on the v0.1.0 ideas list, unscheduled), so this is a visual check only.

### 6.1 Setup
- [ ] Section 0 filled. Build in the lobby is v0.0.41 (the release candidate).
...
- [ ] The replay shows the closing kill from the right angle, with the right fighters, and skips or ends cleanly. Its rewind, play and stop marks are drawn shapes (v0.0.41), not boxes, on every device.
...
- [x] **Ship v0.1.0:** the match ran to the crown, the spectator followed it, nothing on screen was wrong or was only a nit written above. (2026-09-07 on the laptop server: matches 5 and 6, Andrew on the PC vs Bot2, 7 and 9 rounds, both to five crowns and back to the lobby; PC numbers over match 6: draw 240 fps, phys 60, worst frame 19.8 ms, 0 hitches, proc 1.9 ms mean / 3.0 max, phys_cpu 0.6 / 5.6, rtt 4 ms, focus kept. The spectator path was not watched by a person; the harness runs a spectator in every scenario. The replay marks were the one thing found wrong in this play, fixed in v0.0.41.) Run the release runbook, [[release-v0.1.0]] step 2.
