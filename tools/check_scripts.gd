...
# then force-loads every .gd file under res://scripts and res://tools, and
# (v0.1.3) loads and instantiates every .tscn under res://scenes, so a hand-edited
# scene that the editor would refuse fails the bump instead of the game.
#
	...
	for dir_path in SCAN_DIRS:
		for path in _list_scripts(dir_path):
			...
			if not script.can_instantiate():
				failed.append(path)
	var scenes := 0
	for path in _list_files(SCENE_DIR, ".tscn"):
		scenes += 1
		...
		if inst == null:
			...
			inst.free()
	if failed.is_empty():
		print("✅ check_scripts: %d scripts compiled OK, %d scenes load OK" % [checked, scenes])
		quit(0)
	else:
...
func _list_scripts(dir_path: String) -> Array[String]:
	return _list_files(dir_path, ".gd")

func _list_files(dir_path: String, ext: String) -> Array[String]:
	...
	while name != "":
		if not dir.current_is_dir() and name.ends_with(ext) and name != "check_scripts.gd":
